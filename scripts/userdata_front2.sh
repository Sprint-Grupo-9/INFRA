...
if [ ! -d "Front-Tema-Alterado" ]; then
  git clone https://github.com/Sprint-Grupo-9/Front-Tema-Alterado.git || {
...
if [ ! -d "Front-Tema-Alterado" ]; then
  echo "Front-Tema-Alterado does not exist after clone attempt"
  ls -la
  exit 1
fi

cd Front-Tema-Alterado/my-react-app || {
  echo "Failed to enter Front-Tema-Alterado/my-react-app directory"
  ls -la ..
  exit 1
}
